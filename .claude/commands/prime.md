...
## Steps
1. **Sync git state**: `git fetch --all --prune`, `git pull --rebase`, record the current branch via `git rev-parse --abbrev-ref HEAD`, and capture `git status --short`.
2. **Inventory tracked files**: run `git ls-files` (sample representative paths) and note key domains (API, indexer, automation).
3. **Review critical docs**: skim `README.md`, `CLAUDE.md`, `adws/README.md`, and any specs under `specs/` to understand workflow expectations and Bun tooling.
