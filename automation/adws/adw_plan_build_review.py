...
"""Composite workflow that runs planning, build, and review."""
...
def parse_args(argv: list[str]) -> tuple[str, Optional[str]]:
    if len(argv) < 2:
        print("Usage: uv run adws/adw_plan_build_review.py <issue-number> [adw-id]", file=sys.stderr)
        sys.exit(1)
    issue_number = argv[1]
...
def main() -> None:
    ...
    logger = start_logger(adw_id, "adw_plan_build_review")
    logger.info(f"Starting plan+build+review composite | issue #{issue_number} | adw_id={adw_id}")

    steps = ("adw_plan.py", "adw_build.py", "adw_review.py")
    try:
    ...
    logger.info("Plan + build + review workflow completed successfully")
