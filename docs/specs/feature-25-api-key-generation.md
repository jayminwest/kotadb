...
### Problem
The `develop` branch has a complete API key **validation** system (`src/auth/validator.ts`, `middleware.ts`, `cache.ts`) but lacks the **generation** side. Users cannot programmatically create API keys, making the authentication system incomplete. A reference implementation existed on `backup-main-auth-20250108` branch but used an incompatible key format and architecture.
...
### Architecture Notes
This feature creates a new auth module (`src/auth/keys.ts`) that integrates with the existing auth system:
- Uses `getServiceClient()` from `src/db/client.ts` for database writes
- Aligns with `Tier` type from `src/auth/context.ts`
- Generates keys that validate correctly with `validateApiKey()` from `src/auth/validator.ts`
...
- **New**: `src/auth/keys.ts` - Core generation logic
- **Reference**: `src/auth/validator.ts` - Ensure format compatibility
- **Reference**: `src/auth/context.ts` - Use existing `Tier` type
- **Reference**: `src/db/client.ts` - Database client patterns
- **New**: `tests/auth/keys.test.ts` - Comprehensive test suite
...
### Existing Files
- `src/auth/validator.ts` — Key parsing/validation logic; must accept generated keys
- `src/auth/context.ts` — `Tier` type definition; reuse for type safety
- `src/auth/cache.ts` — Validation caching; will cache generated keys after first use
- `src/auth/middleware.ts` — Authentication middleware; will authenticate generated keys
- `src/db/client.ts` — Supabase client patterns; use `getServiceClient()` for writes
- `src/db/migrations/001_initial_schema.sql` — `api_keys` table schema (lines 10-44)
- `tests/auth/validator.test.ts` — Validation tests; should pass with generated keys
- `tests/helpers/db.ts` — Test utilities; reference for test key format

### New Files
- `src/auth/keys.ts` — Core generation functions:
...
- `tests/auth/keys.test.ts` — Integration test suite:
...
Tasks:
- Create `src/auth/keys.ts` with module exports
...
Tasks:
- Create `tests/auth/keys.test.ts` with Supabase Local setup
...
**Validation**: `bun test tests/auth/keys.test.ts` passes
...
**Validation**: `bun test tests/auth/` passes (all auth tests)
...
- Verify migration sync: `bun run test:validate-migrations`
...
Tasks:
- Stage changes: `git add src/auth/keys.ts tests/auth/keys.test.ts`
...
2. Create file: `src/auth/keys.ts` with module structure
3. Create file: `tests/auth/keys.test.ts` with test structure
...
23. Run `bun test tests/auth/keys.test.ts` - verify all tests pass
24. Run `bun test tests/auth/validator.test.ts` - verify compatibility
25. Run `bun test tests/auth/middleware.test.ts` - verify middleware integration
26. Run `bun run lint` - fix any linting issues
27. Run `bun run typecheck` - fix any type errors
28. Run `bun test` - verify full test suite passes
29. Run `bun run build` - verify production build works
30. Run `bun run test:validate-migrations` - verify migration sync
...
### Git & PR Workflow
34. Stage changes: `git add src/auth/keys.ts tests/auth/keys.test.ts`
...
#   - tests/auth/keys.test.ts (new)
#   - tests/auth/validator.test.ts (compatibility)
#   - tests/auth/middleware.test.ts (integration)
...
# Expected: No drift between src/db/migrations and supabase/migrations
```
...
bun run src/auth/keys.ts # If module has CLI entrypoint (optional)
```
