...
**Root Cause:** The `validateApiKey()` function in `src/auth/validator.ts:115` calls `getServiceClient()` internally, creating a new database connection that cannot be mocked without extensive architecture changes.
...
- `package.json` — Add test setup scripts and Supabase CLI integration
- `tests/mcp/errors.test.ts` — Remove mock imports, use real auth
- `tests/mcp/handshake.test.ts` — Remove mock imports, use real auth
- `tests/mcp/tools.test.ts` — Remove mock imports, use real auth
- `tests/api/authenticated-routes.test.ts` — Remove mock imports, use real auth
- `tests/auth/middleware.test.ts` — Remove mock imports, use real database validation
- `tests/auth/validator.test.ts` — Remove mock imports, use real database validation

### Files to Delete
- `tests/helpers/supabase-mock.ts` — Mock Supabase client (88 lines)
- `tests/helpers/auth-mock.ts` — Mock authentication helpers (96 lines)
...
- `tests/helpers/db.ts` — Real database test helpers (e.g., `resetTestDatabase()`, `getTestApiKey()`, `createTestUser()`)
...
2. Create `tests/helpers/db.ts` with real database helpers:
...
1. Update `tests/mcp/errors.test.ts`:
...
1. Update `.github/workflows/ci.yml`:
   ...
   - Run `bun test` against real Supabase local instance
2. Add CI health checks to verify Supabase services are ready before tests

#### Phase 5: Cleanup & Documentation
1. Delete `tests/helpers/supabase-mock.ts`
2. Delete `tests/helpers/auth-mock.ts`
...
- Create `tests/helpers/db.ts` with real database helper functions
...
- Update `tests/mcp/errors.test.ts`: remove mocks, use `getTestApiKey()`
- Update `tests/mcp/handshake.test.ts`: remove mocks, use `getTestApiKey()`
- Update `tests/mcp/tools.test.ts`: remove mocks, use `getTestApiKey()`
- Run `bun test tests/mcp/` to verify MCP tests pass

### 7. Refactor API Tests
- Update `tests/api/authenticated-routes.test.ts`: remove mocks, use `getTestApiKey()`
- Run `bun test tests/api/` to verify API tests pass

### 8. Refactor Auth Tests
- Update `tests/auth/middleware.test.ts`: remove skip conditions, use real validation
- Update `tests/auth/validator.test.ts`: remove skip conditions, use real validation
- Run `bun test tests/auth/` to verify auth tests pass

### 9. Full Test Suite Validation
- Run `bun test` to execute all 93 tests
...
- Delete `tests/helpers/supabase-mock.ts`
- Delete `tests/helpers/auth-mock.ts`
...
- Run `bun test` 10 consecutive times (must pass all runs, 0 flaky tests)
...
- Real database test helpers (`tests/helpers/db.ts`)
- All test files refactored to use real Supabase client (9 test files)
- Mock helpers deleted (`tests/helpers/supabase-mock.ts`, `tests/helpers/auth-mock.ts`)

### Config Updates
- `.env.test` with local Supabase credentials
- `.github/workflows/ci.yml` with Supabase local services
- `package.json` with test setup scripts (if needed)
