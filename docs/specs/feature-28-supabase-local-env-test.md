...
5. **Developer UX polish**: One-command setup (`cd app && bun run test:setup`) with clear output and error messages
...
- Add app/package.json scripts for developer convenience
...
- `app/package.json` — Add `test:setup`, `test:teardown`, `test:reset`, `test:env` scripts
...
- `app/package.json` — Add test lifecycle scripts (test:setup, test:teardown, test:reset, test:env)
...
- `app/tests/**/*.test.ts` — All 94 tests passing, no changes needed
- `app/src/db/migrations/*` — Schema migrations unchanged
...
Tasks:
- Add `"test:setup": "./scripts/setup-test-db.sh"` to app/package.json
- Add `"test:teardown": "supabase stop"` to app/package.json
- Add `"test:reset": "./scripts/reset-test-db.sh"` to app/package.json
- Add `"test:env": "./scripts/generate-env-test.sh"` to app/package.json
- Add `"test:status": "supabase status"` to app/package.json
- Test all new scripts: `cd app && bun run test:setup`, `cd app && bun run test:env`, etc.
...
- Update `docs/testing-setup.md`:
  ...
  - Add "Quick Start" section featuring `cd app && bun run test:setup` (instead of raw script)
...
- Update `README.md` "Running Tests" section:
  - Replace `./scripts/setup-test-db.sh` with `cd app && bun run test:setup`
...
- Verify all 94 tests currently passing: `cd app && bun test`
...
- Link migrations: ensure `supabase/migrations/` contains our migrations or symlink `app/src/db/migrations/`
...
- Run tests to ensure nothing broken: `cd app && bun test`
...
- Test reset script: `cd app && bun run test:reset && bun test`
- Verify tests still pass after reset

### 6. Add Package.json Scripts
- Edit `app/package.json` to add test scripts:
  ```json
  "scripts": {
  ...
  ```
- Test each script:
  - `cd app && bun run test:setup` → Should start Supabase and generate `.env.test`
  - `cd app && bun run test:status` → Should show running services
  - `cd app && bun run test:reset` → Should reset database
  - `cd app && bun run test:teardown` → Should stop services
  - `cd app && bun run test:env` → Should regenerate `.env.test`

### 7. Update Documentation
- Update `docs/testing-setup.md`:
  - Add "Prerequisites" section (Supabase CLI, jq, Docker)
  - Replace "Quick Start" with `cd app && bun run test:setup` workflow
  - Add "Auto-Generated .env.test" section explaining the script
  - Add troubleshooting section for CLI issues
  - Document new app/package.json scripts
  - Keep existing content about test data, RLS, etc.
- Update `README.md` "Running Tests" section:
  - Change first-time setup to `cd app && bun run test:setup`
...
- Full integration test sequence:
  - Stop any running Supabase: `cd app && bun run test:teardown`
  - Fresh setup: `cd app && bun run test:setup`
  - Verify `.env.test` generated correctly
  - Run full test suite: `cd app && bun test` (expect 94 passing)
  - Reset database: `cd app && bun run test:reset`
  - Run tests again: `cd app && bun test` (expect 94 passing)
  - Check status: `cd app && bun run test:status`
  - Teardown: `cd app && bun run test:teardown`
- Measure test execution time (should remain ~11 seconds)
- Test error scenarios:
  - Run `cd app && bun run test:setup` with Docker stopped (should show clear error)
  - Run `cd app && bun run test:env` with Supabase stopped (should show helpful message)

### 10. Validation Commands
- Run type checking: `cd app && bunx tsc --noEmit` (must pass)
- Run linting: `cd app && bun run lint` (must pass)
- Run full test suite: `cd app && bun test` (must pass, 94 tests)
- Run build: `cd app && bun run build` (must pass)
...
### 11. Commit & Push
- Stage changes: `git add scripts/ app/package.json docs/ supabase/ .gitignore`
...
1. `cd app && bun run test:setup` — Setup test environment
2. `cd app && bun test` — Run all 94 tests (must pass)
3. `cd app && bun run test:reset` — Reset database
4. `cd app && bun test` — Run tests again (must pass)
5. `cd app && bun run test:teardown` — Clean up

### Manual Checks
1. **Fresh setup workflow:**
   - Clone repository (simulate new developer)
   - Install dependencies: `bun install`
   - Setup tests: `cd app && bun run test:setup`
...
2. **Supabase CLI integration** — Developer-friendly commands (`cd app && bun run test:setup/reset/teardown`) replacing raw Docker Compose
