...
import type { DependencyEdge, CircularChain } from "../types/dependency.js";

// Simple logger using process.stdout.write
const logger = {
	warn: (message: string, meta?: Record<string, unknown>) => {
		process.stdout.write(JSON.stringify({ level: 'warn', message, ...meta }) + '\n');
	},
	debug: (message: string, meta?: Record<string, unknown>) => {
		process.stdout.write(JSON.stringify({ level: 'debug', message, ...meta }) + '\n');
	}
};
 ...
 * @param filePathById - Map of file IDs to file paths (for descriptions)
 * @param symbolNameById - Map of symbol IDs to symbol names (for descriptions)
 ...
 * process.stdout.write(`Found ${cycles.length} cycles\n`);
 * ```
 */
	...
	if (fileDeps.length > 0) {
		...
		for (const cycle of fileCycles) {
			...
			logger.warn("Detected circular file dependency", {
				cycle: circularChain.description,
			});
		}
	}
