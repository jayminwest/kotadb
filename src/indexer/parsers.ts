...
export async function discoverSources(projectRoot: string): Promise<string[]> {
  ...
  while (pending.length > 0) {
    ...
    try {
      ...
      console.warn(`discoverSources: skipping ${current}: ${(error as Error).message}`);
      continue;
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(`discoverSources: skipping ${fullPath}: ${(error as Error).message}`);
        continue;
      }
...
export async function parseSourceFile(path: string, projectRoot: string): Promise<IndexedFile | null> {
  ...
  try {
    ...
    console.warn(`parseSourceFile: unable to read ${path}: ${(error as Error).message}`);
    return null;
  }
